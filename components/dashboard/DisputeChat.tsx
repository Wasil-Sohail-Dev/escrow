...
import { ArrowLeft, Send, Paperclip, X, Eye, AlertTriangle, CheckCircle, XCircle, Award } from "lucide-react";
...
const DisputeChat = ({ user, userLoading, isAdmin }: { user: any | null, userLoading: boolean, isAdmin: boolean }) => {
  const searchParams = useSearchParams();
  const router = useRouter();
  const disputeId = searchParams.get("disputeId");

  const userId = isAdmin ? user?.id : user?._id;
  ...
  const fetchChat = async (
    ...
    try {
      ...
      setDisputeData({
        title: data?.disputeId?.title || "",
        status: data?.disputeId?.status || "",
      });
  ...
  const fetchDisputes = async () => {
    ...
    try {
      ...
      const response = await fetch(
        `/api/get-dispute?customerId=${userId}&userType=${user.userType}`
      );
  ...
  return (
    <div className="flex flex-col h-screen">
      ...
      <div className={`flex flex-1 overflow-hidden ${isAdmin ? "mt-0" : "mt-[100px]"}`}>
        {/* Disputes Sidebar */}
        {!isAdmin && <div
        ...
        <div
          className={`flex-1 flex flex-col h-full  ${
            !showMobileChat&&!isAdmin ? "hidden md:flex" : "flex"
          }`}
        >
          {disputeId || showMobileChat ? (
            <>
              {/* Chat Header */}
              <div className={`flex ${isAdmin ? "mt-20" : "mt-0"} border-b border-[#DFDFDF] dark:border-dark-border md:justify-between flex-col md:flex-row items-start md:items-center px-4`}>
                <div className="h-[90px] flex items-center px-4 flex-shrink-0 bg-white dark:bg-dark-bg">
                  {showMobileChat && (
                    <button
                ...
                <div className="flex items-center gap-4">
                  <span className={`px-3 py-1 text-small-medium rounded-full ${getStatusClass(disputeData.status)}`}>
                    {disputeData.status.charAt(0).toUpperCase() + disputeData.status.slice(1)}
                  </span>
                  {isAdmin && disputeData.status !== "resolved" && disputeData.status !== "rejected" && (
                    <Button
                      onClick={() => setIsResolutionModalOpen(true)}
                      className="bg-primary hover:bg-primary/90 text-white dark:text-dark-text"
                    >
                      Resolve Dispute
                    </Button>
                  )}
      ...
      {/* Replace the Dialog with the new modal */}
