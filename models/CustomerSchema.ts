...
const customerStatusTransitions: CustomerStatusTransitions = {
    ...
    active: ["adminInactive", "userInactive"],
    adminInactive: ["verified"],
    userInactive: ["active", "adminInactive"],
};

const isValidCustomerStatusTransition = (currentStatus: string, newStatus: string): boolean => {
...
interface ICustomer extends mongoose.Document {
    ...
    userStatus: "pendingVerification" | "verified" | "active" | "adminInactive" | "userInactive";
    onboardingToken?: string;
    tokenExpiresAt?: Date;
    updateStatus(newStatus: string): Promise<void>;
}

customerSchema.methods.updateStatus = async function (this: ICustomer, newStatus: string): Promise<void> {
    ...
    this.userStatus = newStatus as "pendingVerification" | "verified" | "active" | "adminInactive" | "userInactive";
    await this.save();
};
